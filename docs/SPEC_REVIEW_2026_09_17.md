...
| G10 | §3.5 | `round(x, 4)` does not pin a rounding mode. Python is half-even; most languages are half-up. A tie moves a derived value by 5e-5 — five decades above the pinned 1e-9 tolerance — so a tie flips the verdict. |
