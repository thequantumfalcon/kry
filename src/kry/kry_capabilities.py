...
CAPABILITIES: tuple = (
    ...
    Capability("external_chain_head_anchor", "implemented",
               symbols=("kry.kry_mint:export_chain_anchor", "kry.kry_mint:verify_chain_against_anchor",
                        "kry.kry_settlement:export_registry_anchor",
                        "kry.kry_settlement:verify_registry_against_anchor"),
               files=("scripts/kry_chain_anchor.py",),
               tests=("tests/test_veracity_tier.py", "tests/test_external_verify.py"),
               honest_limit="Detects a re-mint (mint chain) or rollback/un-spend (settlement) ONLY against an "
                            "anchor the operator PUBLISHED externally; an anchor handed over at verify time proves nothing."),
    ...
    Capability("double_spend_and_rollback_guard", "implemented",
               ...
               honest_limit="Single-HOST multi-process atomic (commit-time ceiling re-check) + tail-truncation "
                            "guard (HOLE F) + published registry anchor; cross-NODE is post-facto (HOLE D)."),
