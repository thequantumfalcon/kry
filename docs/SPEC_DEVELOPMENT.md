...
| v1.3 | 2026-08-01 | §3.5 made **derivable from the spec alone**: a present-but-non-object `veracity` is INVALID; `by_tier` compared as a map with equal key sets; all four §3.5 fields required (absent ≠ a declared zero); and the numeric comparison tolerance pinned at **1e-9** as a number, replacing "tolerance as in the reference". Ten vectors added. Both implementations pass 46/46 and the 10⁶ differential fuzz at the expanded mutation space returns 0 divergences. |
